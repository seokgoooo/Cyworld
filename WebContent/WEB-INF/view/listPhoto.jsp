	...
	<table border="1">
		...
		<c:forEach var="photo" items="${photoPage.content}">
			<tr>
				...
				<td><a
					href="read.do?no=${photo.photoNum}&pageNo=${photoPage.currentPage}">
		...
		<c:if test="${photoPage.hasPhoto()}">
			...
			<tr>
				<td colspan="4"><c:if test="${photoPage.startPage > 5}">
						<a href="list.do?pageNo=${photoPage.startPage -5}">[이전]</a>
					</c:if> <c:forEach var="pNo" begin="${photoPage.startPage}"
						end="${photoPage.endPage }">
						<a href="list.do?pageNo=${pNo}">[${pNo}]</a>
					</c:forEach> <c:if test="${photoPage.endPage < photoPage.totalPages }">
