...
<body>
	<table border="1">
		...
		<c:if test="${ ownerPage.hasOwner() }">
			<tr>
				<td colspan="4"><c:if test="${ ownerPage.startPage > 5 }">
						<a href="list.do?pageNo=${ ownerPage.startPage - 5 }">[이전]</a>
					</c:if> <c:forEach var="pNo" begin="${ ownerPage.startPage }"
						end="${ ownerPage.endPage }">
						<a href="list.do?pageNo=${ pNo }">[${ pNo }]</a>
					</c:forEach> <c:if test="${ ownerPage.endPage < ownerPage.totalPages }">
